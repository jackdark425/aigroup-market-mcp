  ...
  async run(args: { code: string; market_type: string; start_datetime: string; end_datetime: string; freq: string }) {
    try {
      ...
      while (cursor < endMs && pageIndex < maxPages) {
        const url = `${config.api.binance.baseUrl}/api/v3/klines?symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(interval)}&startTime=${cursor}&endTime=${endMs}&limit=${config.pagination.defaultPageSize}`;
        ...
        pageIndex += 1;
        if (kl.length < config.pagination.defaultPageSize) break;
      }
